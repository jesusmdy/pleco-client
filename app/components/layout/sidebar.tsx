  ...
  return (
    <div className="w-[240px] flex flex-col hidden md:flex bg-md-surface-container-low h-full border-r border-md-outline-variant/10 shadow-2xl z-10">
      <div className="flex-1 p-3 flex flex-col gap-2 mt-4">
        <Link
          href="/fm/drive"
          className={`rounded-2xl px-5 py-3 flex items-center gap-3.5 transition-all group ${isDriveActive ? 'bg-md-primary-container text-md-on-primary-container font-bold shadow-md shadow-black/20' : 'bg-transparent text-md-on-surface-variant hover:bg-md-surface-variant/20 hover:text-md-on-surface'}`}
        >
          <HardDrive className={`w-5 h-5 ${isDriveActive ? 'text-md-primary' : 'text-md-on-surface-variant group-hover:text-md-on-surface'}`} />
          <span className="text-[14px] uppercase tracking-widest">My Drive</span>
        </Link>

        <Link
          href="/fm/drive/trash-bin"
          className={`rounded-2xl px-5 py-3 flex items-center gap-3.5 transition-all group ${isTrashActive ? 'bg-md-primary-container text-md-on-primary-container font-bold shadow-md shadow-black/20' : 'bg-transparent text-md-on-surface-variant hover:bg-md-surface-variant/20 hover:text-md-on-surface'}`}
        >
          <Trash2 className={`w-5 h-5 ${isTrashActive ? 'text-md-primary' : 'text-md-on-surface-variant group-hover:text-md-on-surface'}`} />
          <span className="text-[14px] uppercase tracking-widest">Trash</span>
        </Link>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between text-md-on-surface-variant group-hover:text-md-on-surface transition-colors">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] font-bold uppercase tracking-widest">Storage</span>
          </div>
        ...
        <div className="text-[12px] text-md-on-surface-variant leading-relaxed">
          Using <span className="text-md-on-surface font-bold">{formatBytes(used)}</span><br />
